...
export default function App() {
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      ...
      // Play tick sound in last 5 seconds (but not during getReady countdown)
      if (gameState.phase !== 'getReady' && remaining <= 5 && remaining > 0 && remaining !== lastTickRef.current) { 
        soundManager.playTick(); 
        lastTickRef.current = remaining; 
      }
      // Play tick during getReady for countdown effect
      if (gameState.phase === 'getReady' && remaining > 0 && remaining !== lastTickRef.current) {
        soundManager.playTick();
        lastTickRef.current = remaining;
      }
      if (remaining === 0 && !timerExpiredRef.current) { 
        timerExpiredRef.current = true; 
        // Only play time up sound if not transitioning from getReady
        if (gameState.phase !== 'getReady') soundManager.playTimeUp(); 
  ...
  const handleTimeUp = async () => { 
    if (!isHost) return;
    if (gameState?.phase === 'getReady') {
      // Transition from getReady to buzzer phase
      await update(ref(db, `rooms/${roomCode}/game`), { phase: 'buzzer', timerEnd: Date.now() + 30000 });
  ...
  const startRound = async (category) => {
    ...
    await update(ref(db, `rooms/${roomCode}/game`), {
      ...
      phase: 'getReady', buzzedPlayer: null, timerEnd: Date.now() + 5000, answers: {}, tournament: null, penaltyMode
    });
  };
  ...
  const startTournament = async () => {
    ...
    await update(ref(db, `rooms/${roomCode}/game`), {
      ...
      phase: 'getReady', buzzedPlayer: null, timerEnd: Date.now() + 5000, answers: {}, penaltyMode,
  ...
  const submitAnswer = async (answer) => {
    ...
    const penalty = gameState.penaltyMode ? -50 : 0;
    const points = correct ? 100 + Math.max(0, localTimer * 3) : penalty;
    ...
    if (correct) {
      ...
      const cat = gameState.category;
      const newScore = Math.max(0, (players[playerId]?.score || 0) + penalty);
      const newCatScore = Math.max(0, (players[playerId]?.categoryScores?.[cat] || 0) + penalty);
      await update(ref(db, `rooms/${roomCode}/players/${playerId}`), { score: newScore, [`categoryScores/${cat}`]: newCatScore });
    }
  ...
  const nextQuestion = async () => {
    ...
    if (nextIdx >= gameState.questions.length) {
      if (t && t.currentCategoryIndex < t.totalCategories - 1) {
        ...
        await update(ref(db, `rooms/${roomCode}/game`), { category: nextCat.category, roundType: meta.type || 'standard', questions: nextCat.questions, questionIndex: 0, currentQuestion: nextCat.questions[0], phase: 'getReady', buzzedPlayer: null, timerEnd: Date.now() + 5000, answers: {}, 'tournament/currentCategoryIndex': nextCatIdx });
      } else { soundManager.playVictory(); await update(ref(db, `rooms/${roomCode}/game`), { status: t ? 'tournamentEnd' : 'roundEnd', phase: null }); }
    } else {
      await update(ref(db, `rooms/${roomCode}/game`), { questionIndex: nextIdx, currentQuestion: gameState.questions[nextIdx], phase: 'getReady', buzzedPlayer: null, timerEnd: Date.now() + 5000, answers: {} });
    }
  };
          ...
          showTournamentSetup ? (
            <div className="bg-white/10 rounded-xl p-4">
              ...
              <div className="mb-3 flex items-center justify-between bg-white/10 rounded-lg p-2">
                <span className="text-white text-sm flex items-center gap-2">⚠️ Wrong answer penalty (-50)</span>
                <button onClick={() => setPenaltyMode(!penaltyMode)} className={`px-3 py-1 rounded-full text-xs font-bold ${penaltyMode ? 'bg-red-500 text-white' : 'bg-white/20 text-white/60'}`}>{penaltyMode ? 'ON' : 'OFF'}</button>
              </div>
            ...
            <div>
              ...
              <div className="mb-2 flex items-center justify-center gap-2">
                <span className="text-white/70 text-xs">Penalty:</span>
                <button onClick={() => setPenaltyMode(!penaltyMode)} className={`px-2 py-1 rounded text-xs font-bold ${penaltyMode ? 'bg-red-500 text-white' : 'bg-white/20 text-white/60'}`}>{penaltyMode ? 'ON (-50)' : 'OFF'}</button>
              </div>
      ...
      <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 p-4">
        <div className="max-w-lg mx-auto">
          <div className="flex justify-between items-center mb-3">
            <div className="flex items-center gap-1"><span className="text-xl">{meta.icon}</span><span className="text-white text-sm truncate max-w-20">{meta.name}</span>{gameState?.penaltyMode && <span className="text-red-400 text-xs ml-1">⚠️</span>}</div>
            <div className="flex items-center gap-2">
              ...
              <div className={`px-2 py-1 rounded-full text-white flex items-center gap-1 text-sm ${phase === 'getReady' ? 'bg-yellow-500' : localTimer <= 5 ? 'bg-red-500 animate-pulse' : 'bg-white/20'}`}>
                <Clock className="w-3 h-3" />
                {phase === 'getReady' ? `${localTimer}...` : `${localTimer}s`}
              </div>
            </div>
          </div>
          ...
          {phase === 'getReady' && (
            <div className="mb-3">
              <div className="bg-gradient-to-r from-yellow-500 to-orange-500 text-white rounded-2xl p-6 text-center shadow-lg">
                <p className="text-lg font-bold mb-2">🎯 GET READY!</p>
                <div className="text-6xl font-bold animate-pulse">{localTimer}</div>
                <p className="text-sm mt-2 opacity-80">Buzzer opens soon...</p>
              </div>
            </div>
          )}
          ...
          {phase === 'answering' && buzzed && (
            <div className="mb-3">
              <div className="bg-yellow-500/20 border-2 border-yellow-500 rounded-xl p-3 mb-3">
                <p className="text-center text-yellow-400 font-bold text-lg">🔔 {players[buzzed]?.name} buzzed first!</p>
              </div>
              ...
              )) : (<div className="text-center text-white bg-white/10 rounded-xl p-4"><div className="text-3xl mb-1">👀</div><p>{timeUp ? "Time ran out!" : "Waiting for answer..."}</p></div>)}
            </div>
          )}
          {phase === 'reveal' && (
            <div className="mb-3">
              {/* Show who answered and their result */}
              {buzzed && gameState?.answers?.[buzzed] && (
                <div className={`rounded-xl p-4 mb-3 ${gameState.answers[buzzed].correct ? 'bg-green-600' : 'bg-red-600'}`}>
                  <div className="flex items-center justify-center gap-2 text-white mb-2">
                    <span className="text-2xl">{gameState.answers[buzzed].correct ? '✅' : '❌'}</span>
                    <span className="font-bold text-lg">{players[buzzed]?.name}</span>
                  </div>
                  <p className="text-white/80 text-center text-sm">answered:</p>
                  <p className="text-white text-center font-bold text-lg">"{gameState.answers[buzzed].answer}"</p>
                  {gameState.answers[buzzed].correct ? (
                    <p className="text-yellow-300 text-center text-sm mt-1">+{gameState.answers[buzzed].points} points!</p>
                  ) : gameState.penaltyMode ? (
                    <p className="text-yellow-300 text-center text-sm mt-1">−50 points penalty!</p>
                  ) : null}
                </div>
              )}
              {/* Show if no one answered */}
              {!buzzed && (
                <div className="bg-gray-600 rounded-xl p-4 mb-3 text-center">
                  <p className="text-white text-lg">⏱️ No one buzzed in time!</p>
                </div>
              )}
              {/* Show correct answer */}
              <div className="bg-white rounded-xl p-4 text-center mb-3 border-2 border-green-500">
                <p className="text-gray-500 text-sm">Correct Answer:</p>
                <p className="text-2xl font-bold text-green-600">{q?.answer}</p>
              </div>

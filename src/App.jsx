    ...
    <div className="min-h-screen bg-gradient-to-b from-red-900 via-red-800 to-green-900 p-4 relative">
      ...
      <div className="max-w-md mx-auto relative z-10 pt-6">
        ...
        <div className="bg-white/10 backdrop-blur rounded-xl p-3 mb-4">
          <input type="text" value={playerName} onChange={(e) => setPlayerName(e.target.value)} placeholder="Your name" className="w-full px-4 py-2 rounded-lg bg-white/20 text-white placeholder-white/50 text-lg text-center font-medium" maxLength={15} />
        </div>
        <div className="space-y-3">
          <div className="bg-white/10 backdrop-blur rounded-xl p-3">
            <div className="flex items-center gap-2 mb-2">
              <input type="checkbox" id="usePass" checked={usePassword} onChange={(e) => setUsePassword(e.target.checked)} className="w-4 h-4" />
              <label htmlFor="usePass" className="text-white text-sm flex items-center gap-1"><Lock className="w-4 h-4" /> Password protect room</label>
            </div>
            {usePassword && (
              <input type="text" value={roomPassword} onChange={(e) => setRoomPassword(e.target.value)} placeholder="Set room password" className="w-full px-3 py-2 rounded-lg bg-white/20 text-white placeholder-white/50 text-sm mb-2" maxLength={20} />
            )}
            <button onClick={createGame} disabled={!connected} className="w-full bg-green-600 hover:bg-green-500 disabled:bg-gray-600 text-white font-bold py-3 px-4 rounded-xl flex items-center justify-between active:scale-95 transition-transform">
              <span className="flex items-center gap-2"><Crown className="w-5 h-5" />Host New Game</span><ChevronRight className="w-5 h-5" />
            </button>
          </div>
          <div className="relative"><div className="absolute inset-0 flex items-center"><div className="w-full border-t border-white/20"></div></div><div className="relative flex justify-center"><span className="px-4 text-white/60 text-sm">or join</span></div></div>
          <div className="bg-white/10 backdrop-blur rounded-xl p-3">
            <input type="text" value={joinCode} onChange={(e) => setJoinCode(e.target.value.toUpperCase().slice(0, 4))} placeholder="ROOM CODE" className="w-full px-4 py-2 rounded-lg bg-white/20 text-white placeholder-white/50 text-xl text-center font-mono tracking-widest uppercase mb-2" maxLength={4} />
            <input type="text" value={joinPassword} onChange={(e) => setJoinPassword(e.target.value)} placeholder="Password (if required)" className="w-full px-3 py-2 rounded-lg bg-white/20 text-white placeholder-white/50 text-sm mb-2" />
            <button onClick={joinGame} disabled={!connected || joinCode.length !== 4} className="w-full bg-blue-600 hover:bg-blue-500 disabled:bg-gray-600 text-white font-bold py-2 rounded-lg active:scale-95 transition-transform">Join Game</button>
          </div>
        </div>
